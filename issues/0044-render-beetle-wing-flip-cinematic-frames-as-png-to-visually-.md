...
- 2026-05-04: closing — visual confirmation complete.
  ImageMagick `convert` is available in the environment and the
  SVG→PNG step works. Rendered 29 distinct DOS BEETLE polygon
  offsets (CINEMATIC_BEETLE_WALKING_*, _DRIFT_*, _FLY_AWAY_*,
  _LIFT_*, _FLYING_*, _HOVER_*, _HOVER_VERT_*) to PNG at zoom 256
  with a dark background. Built per-animation horizontal sprite
  sheets at `docs/assets/research-05-beetle-frames/`.

  Visual reading of the DOS sequences:

  - `walking_right_sequence.png`: 7-frame walk cycle, antennae +
    body + legs unambiguously a beetle. Same shape research/05
    discussed.
  - `drift_right_sequence.png`: 5 frames of beetle tipping from
    upright (frame 0) to fully-on-its-back (frame 4) — exactly
    the "fall onto back after kick" pose research/05 hypothesized.
  - `fly_away_sequence.png`: 4 frames of beetle lying on back,
    legs flailing — the "stunned" loop.
  - `beetle_lift_sequence.png` + `beetle_flying_sequence.png`:
    additional rotated/flying poses (the names from semantic-
    rename round 16 are consistent with the visual content).

  research/05's "Visual confirmation" subsection now embeds
  the walking_right sprite sheet inline and references the
  remaining 6 sprite sheets in `docs/assets/research-05-beetle-
  frames/`. The issue's qualitative claim ("beetle has wings
  opening, flapping, and falls onto back") is now backed by
  rendered evidence.

  Sprite sheets generated by:
    python3 tools/polygon_render.py <PCS> <off> --bg #444 --zoom 256
    convert <svg> <png>
    convert frame0..N +append <sequence>.png
