#!/usr/bin/env python3
"""Sync cart per-branch's named routines to dos/amiga per-branch.

For each LABEL_<HEX> in dos/amiga per-branch, look for a routine in
cart per-branch with the same abstracted body, and apply that name."""
import re
import sys
...
def main():
    if len(sys.argv) < 2:
        sys.exit("usage: STAGE [aggressive|conservative]")
    
    stage = sys.argv[1].upper()
    aggressive = len(sys.argv) > 2 and sys.argv[2] == 'aggressive'
    
    if not aggressive:
        # Use conservative abstraction
        def aggressive_abstract_local(body):
            out = []
            for ln in body:
                s = re.sub(r';@raw=[^;]*$', '', ln).rstrip()
                if not s.strip():
        ...
        aggressive_abstract_local = aggressive_abstract
    
    cart_path = AW_SRC / f"src/levels/cartridge_1992/{stage}.asm"
    if not cart_path.exists():
        sys.exit(f"FATAL: no cart per-branch {cart_path}")
    ...
    for label, body in parse_routines(cart_text):
        if label.startswith('LABEL_') or label.startswith('JUNK_'):
            continue
        sym = aggressive_abstract_local(body)
        if sym:
            cart_named.setdefault(sym, []).append(label)
    
    print(f"cart {stage}: {len(cart_named)} unique named bodies", file=sys.stderr)
    
    for branch in ('dos_1992', 'chahi_amiga_1991'):
