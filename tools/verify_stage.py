...
def assemble(asm_path: Path) -> bytes:
    # Per-branch sources may use:
    #   - `;@include "<path>"` to pull in shared headers like
    #     `_common_vars.inc` (var-alias EQUs shared across branches).
    #   - `FILL(n, 0xXX)` for trailing-padding regions.
    # Both are expanded via tools/awvm_preprocess.py before assembly.
    # Per-branch files don't carry `;@if BRANCH ==` directives (those
    # live in the unified `_unified/*.asm.in` files), so flag-based
